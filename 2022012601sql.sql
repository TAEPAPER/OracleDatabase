    ...
    1.SYNONYM(동의어)
    -오라클 객체에 부여하는 별칭
    -긴 이름의 객체나 다른사람 소유의 객체에 접근할 때 주로 사용
    ...
    CREATE [OR REPLACE] SYNONYM 동의어
     ...
     SELECT * FROM DEPT;
      ...
      SELECT SYSDATE FROM MYDUAL;  --객체라는 것들에는 모두 별칭을 부여해서 별도의 이름으로 불러다 쓸 수 있음
    ...
    -인덱스의 종류
    .UNIQUE/NON-UNIQUE : 인덱스가 중복값을 허용하는지 여부에 따른 분류
      'UNIQUE'인덱스는 NULL값도 허용하나 하나의 NULL만 허용됨--PK인덱스는 유니크 인덱스당
    .SINGLE/COMPOSITE : 인덱스 구성 컬럼이 1개인경우(SINGLE), 2개 이상의 컬럼으로 구성(COMPOSITE) 된 경우
    .NORMAL INDEX : DEFAULT 인덱스로 컬럼값과 ROWID(물리적 위치정보)를 기반으로 주소가 계산되며 트리구조 이용
    .BITMAP INDEX : 컬럼값과 ROWID(물리적 위치정보)를 2진으로 조합하여 주소계산하며, CARDINALITY(도메인종류)가 적은 경우
                    (성별,나이 등) 효율적인 방식
    .FUNCTION-BASED NORMAL INDEX : 인덱스 구성컬럼에 함수가 적용된 경우로 이 인덱스를 이용하여 자료를 검색하는 경우 
     ...
     (사용형식)
        ...
        .'ASC|DESC' :  인덱스 생성이 정렬 방식(기본은 ASC)
