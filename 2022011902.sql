    ...
    **HR 계정에 급여에 따른 등급표를 작성하시오
    ...
    사용예) HR계정의 사원테이블에서 급여에 따른 등급을 조회하여 출력하시오
          ...
          WHERE A.DEPARTMENT_ID = B.DEPARTMENT_ID --동등조인
                AND (A.SALARY >= C.LOW_SAL AND A.SALARY <= C.MAX_SAL)--논 이퀴 조인
            ORDER BY 3;
     ...
     사용예) 사원테이블에서 사원들의 평균급여보다 많은 급여를 받는 사원들을 조회하시오
        ...
        B.JOB_TITLE AS 직무명, 
        A.SALARY AS 급여
        FROM HR.EMPLOYEES A , HR.JOBS B, (SELECT AVG(SALARY) AS ASAL FROM HR.EMPLOYEES) C
    ...
    숙제)사원테이블에서 **부서별** 평균임금을 구하고 해당부서에 속한 사원 중 자기부서의 평균 급여보다 많은 급여를 받는 사원을 조회하시오
        ...
        제출일자 : 2022년 1월 28일
        제출방법 : 파일전송(SEM-PC의 D:\공유폴더\Oracle\homework01)
        파일명 : 메모장 등을 활용하여 txt 또는 doc  또는 hwp 파일로 저장하여 전송
        파일명은 이름작성일자.txt( ex  홍길동20220119.txt)
