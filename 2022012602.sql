2022-0126-02) PL/SQL(Procedural Language SQL)
    ...
    - block 구조로 구성됨
    - DBMS에 미리 컴파일되어 저장되므로 빠른 실행과
      네트워크를 효율적으로 이용하여 전체 SQL실행 효율을 증대
    - 변수, 상수, 반복처리, 비교판단, 에러처리 가능
    - 표준 문법이 없음
    - User Defined function, Stored Procedure, Trigger, 
      package, Anonymous block 등이 제공 --function은 반환값이 있음, procedure는 반환값 없음.
    ...
    1)변수(상수)선언
       ...
       .데이터타입 : 표준 SQL에서 사용하는 데이터타입, 
                    PLS_INTEGER, 
                    BINARY_INTEGER : 4BYTE 정수
                    BOOLEAN 타입 사용 가능 ( true, false , null)
        **참조타입 -> 데이터 타입 대신 기술
        테이블명.컬럼명%TYPE : 해당 테이블의 컬럼과 같은 타입/크기로 변수(상수)를 선언 
        테이블명%ROWTYPE: 해당 테이블의 한 행 전체와 같은 타입으로 변수선언(C언어의 STRUCTURE 타입과 유사)
        .'NOT NULL' 지정시 반드시 초기값 지정해야함
        --배열과 비슷한 개념의 변수가 제공되어진다!!
        --SELECT INTO FROM WHERE
    사용예) 임의의 부서번호(10-110번)를 생성하고 해당부서에 속한 사원 중 입사일이 가장빠른 
            사원번호, 사원명, 입사일,부서명을 출력
    
    DECLARE
    V_DEPTNO HR.DEPARTMENTS.DEPARTMENT_ID%TYPE; --부서코드가 저장될 변수
    V_NAME HR.EMPLYOEES.EMP_NAME%TYPE; --사원명
    ...
    SELECT TBL.AID, TBL.ANAME , TBL.ADATE, TBL.BNAME
     INTO V_EID,V_ENAME,V_HDATE,V_HNAME  --서로 개수의 맵핑이 되지 않는다 => 커서를 통해서 해결한다
    FROM 
    (SELECT A.EMPLOYEE_ID AS AID,
            A.EMP_NAME AS ANAME,
    ...
    DBMS_OUTPUT.PUT_LINE('부서명 : '||V_HNAME);
    DBMS_OUTPUT.PUT_LINE('------------------');   
    END;
        
    사용예)길이를 하나 입력받아 그 길이를 반지름으로하는 원의 너비, 그 길이를 한 변으로하는 정사각형의
          너비를 각각 구하시오
          
    ACCEPT P_LEN PROMPT '길이를 입력 : ' --이 변수에 자동으로 저장되어진다
    DECLARE
     V_LENGTH NUMBER := TO_NUMBER('&P_LEN'); --&내용을 검사하십시오
     V_SEQUARE NUMBER :=0;--사각형 너비    --숫자 반드시 초기화 해줘야해(선언만하면 안돼!)
